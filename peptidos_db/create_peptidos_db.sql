...
CREATE TABLE PEPTIDO_ORGANISMO (
    ...
    mic_valor JSONB,  -- almacena {valor, unidad}
...
SELECT p.id, p.nombre_principal,
       jsonb_agg(DISTINCT jsonb_build_object('organismo', ob.nombre_cientifico, 'mic', po.mic_valor)) AS organismos,
...
-- Crear roles
CREATE ROLE administrador WITH LOGIN SUPERUSER CREATEDB CREATEROLE;
CREATE ROLE curador WITH LOGIN PASSWORD '12345678';
CREATE ROLE consultor WITH LOGIN;
